...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const [recentResult, patientResult, totalResult, gliomaResult, meningiomaResult, pituitaryResult, noTumorResult, uncertaintyResult] = await Promise.all([
        ...
        const firstError = [recentResult.error, patientResult.error, totalResult.error, gliomaResult.error, meningiomaResult.error, pituitaryResult.error, noTumorResult.error, uncertaintyResult.error].find(Boolean)
        ...
        const confidence = uncertaintyRows.length
          ? uncertaintyRows.reduce((sum, row) => sum + Number(row.uncertainty?.confidence || 0), 0) / uncertaintyRows.length
          : 0
        ...
        const mappedPatients = ((patientResult.data || []) as any[]).map(patient => ({
          ...patient,
          prediction_count: patient.predictions?.[0]?.count || 0,
        }))
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Link to="/upload" className="btn-primary"><Upload className="h-4 w-4" />New Scan</Link>}
      />
        ...
        {cards.map(([label, value, Icon, bg, color]) => (
          <div key={label} className="card p-5">
            <div className="flex items-center justify-between">
              <div><p className="text-sm text-neutral-500">{label}</p><p className="mt-1 text-2xl font-bold">{value}</p></div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 card p-5">
          <div className="mb-4 flex justify-between"><h2 className="text-lg font-semibold">Recent Analyses</h2><Link to="/history" className="flex items-center text-sm text-primary-600">View all<ChevronRight className="h-4 w-4" /></Link></div>
          {loading ? <div className="space-y-3">{[1, 2, 3].map(i => <div key={i} className="h-24 rounded-lg shimmer-bg" />)}</div> : recent.length ? <div className="space-y-3">{recent.map(p => <PredictionCard key={p.id} prediction={p as PredictionResult} />)}</div> : <div className="py-12 text-center"><Activity className="mx-auto mb-3 h-10 w-10 text-neutral-400" /><p>No analyses yet</p><Link to="/upload" className="btn-primary mt-4"><Upload className="h-4 w-4" />Upload MRI</Link></div>}
        </div>

        <div className="space-y-6">
          <div className="card p-5">
            <h2 className="mb-5 text-lg font-semibold">Tumor Distribution</h2>
            <div className="flex items-center gap-6">
              <div className="relative h-40 w-40 shrink-0 rounded-full" style={pieStyle} aria-label="Tumor distribution pie chart">
                <div className="absolute inset-8 rounded-full bg-white" />
              </div>
              <div className="min-w-0 flex-1 space-y-3">
                {(Object.keys(distribution) as TumorClass[]).map(key => {
                  const count = distribution[key]
                  const total = Object.values(distribution).reduce((sum, value) => sum + value, 0)
                  const percentage = total ? (count / total) * 100 : 0
                  return <div key={key} className="flex items-center justify-between gap-3 text-sm">
                    <div className="flex min-w-0 items-center gap-2"><span className="h-3 w-3 shrink-0 rounded-full" style={{ backgroundColor: TUMOR_CLASS_COLORS[key] }} /><span className="truncate">{TUMOR_CLASS_DISPLAY[key]}</span></div>
                    <span className="shrink-0 text-neutral-500">{count} ({percentage.toFixed(0)}%)</span>
                  </div>
                })}
              </div>
            </div>
          ...
          <div className="card p-5">
            ...
            {patients.length ? patients.map(p => <Link key={p.id} to={`/history?patient=${p.id}`} className="flex items-center justify-between rounded-lg px-3 py-2.5 hover:bg-neutral-50"><div><p className="text-sm font-medium">{p.name}</p><p className="text-xs text-neutral-400">MRN: {p.mrn}</p></div><span className="badge bg-primary-50 text-primary-600">{p.prediction_count || 0} scans</span></Link>) : <p className="py-8 text-center text-sm text-neutral-400">No patients registered yet</p>}
          </div>
        </div>
      </div>
    </div>
