...
export async function checkBackendHealth(): Promise<void> {
  ...
  try {
    ...
    if (!data.model_loaded) throw new Error('AI backend is running, but the trained model is not loaded.')
  } catch (error: any) {
    if (error?.name === 'AbortError') throw new Error('AI backend health check timed out. Please make sure the backend is running.')
    if (error instanceof TypeError) throw new Error('AI backend is unavailable. Start the FastAPI server at http://localhost:8000 and try again.')
    throw error
  } finally {
...
export async function runInference(file: File, patientId: string): Promise<PredictionResult> {
  ...
  try {
    response = await fetch(`${API_URL}/api/v1/predict`, {
      ...
      body: JSON.stringify({ image_base64: imageBase64, patient_id: patientId }),
    })
  } catch {
  ...
  if (!response.ok) {
    ...
    try { detail = JSON.parse(responseText).detail || responseText } catch { /* keep raw response */ }
...
export function normalizePredictionResult(raw: any): PredictionResult {
  ...
  return {
    ...raw,
    probabilities,
    predicted_class_display: raw?.predicted_class_display ?? raw?.predicted_class,
    uncertainty: {
      ...raw?.uncertainty,
...
function fileToBase64(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      if (typeof reader.result !== 'string') return reject(new Error('Failed to read uploaded image'))
