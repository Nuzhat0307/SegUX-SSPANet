...
async def create_patient(patient: PatientCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    p = Patient(user_id=current_user.id, **patient.model_dump()); db.add(p); db.commit(); db.refresh(p)
...
async def list_patients(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return [PatientResponse(id=str(p.id), name=p.name, age=p.age, gender=p.gender, mrn=p.mrn, notes=p.notes, created_at=p.created_at) for p in patients]
