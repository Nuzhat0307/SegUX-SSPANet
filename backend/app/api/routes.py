...
async def register(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    user = User(email=user_data.email, full_name=user_data.full_name, hashed_password=get_password_hash(user_data.password))
...
async def predict(request: PredictRequest):
    try:
        ...
        return PredictionResponse(
            id=str(uuid4()), patient_id=str(request.patient_id), image_base64=request.image_base64,
            predicted_class=result["predicted_class"], predicted_class_display=result["predicted_class_display"],
            probabilities=result["probabilities"], uncertainty=result["uncertainty"], segmentation=result["segmentation"],
            gradcam_results=result["gradcam_results"], feature_explanation=result.get("feature_explanation"),
            model_version=result["model_version"], inference_time_ms=result["inference_time_ms"],
            created_at=result.get("created_at") or datetime.now(timezone.utc), notes=result.get("notes"),
        )
    except Exception as e:
        logger.exception("Trained inference failed")
...
async def list_predictions(limit: int = 50, offset: int = 0, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    preds = db.query(Prediction).filter(Prediction.user_id == current_user.id).order_by(Prediction.created_at.desc()).offset(offset).limit(limit).all()
...
async def generate_report(prediction_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if report is None:
        report = Report(user_id=current_user.id, prediction_id=prediction_id, report_type="full")
...
async def list_reports(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return [ReportResponse(id=str(r.id), prediction_id=str(r.prediction_id), report_type=r.report_type, created_at=r.created_at) for r in reports]

def _prediction_to_response(pred: Prediction) -> PredictionResponse:
    return PredictionResponse(
        id=str(pred.id), patient_id=str(pred.patient_id), image_base64=pred.image_base64,
        predicted_class=pred.predicted_class, predicted_class_display=pred.predicted_class_display,
        probabilities=pred.probabilities, uncertainty=pred.uncertainty, segmentation=pred.segmentation,
        gradcam_results=pred.gradcam_results, feature_explanation=getattr(pred, "feature_explanation", None),
        model_version=pred.model_version, inference_time_ms=pred.inference_time_ms,
        created_at=pred.created_at, notes=pred.notes,
    )
